...
public class ata5 : MonoBehaviour
{
    ...
    private string targetWord = "Gözlerini Aç"; // Hedef kelime
        ...
        foreach (char c in Input.inputString)
        {
            ...
            else if ((c == '\n') || (c == '\r')) // Enter
            {
                if (typedWord == targetWord)
                {
                    ...
                    StartCoroutine(LoadSpecificSceneAfterDelay(1)); // 1 saniye sonra belirli sahneye geçiþ yap
                }
                else
                {
    ...
    {
        yield return new WaitForSeconds(delay);
        SceneManager.LoadScene("Giris"); // "Giris" adlý sahneye geçiþ yap
    }
}
